...
async function fixGDInPricing() {
  try {
    ...
    // Remove ANY existing GD services (old and new to start fresh)
    updatedServices = updatedServices.filter(s => 
      !s.id.toLowerCase().includes('gd')
    );
    ...
    console.log('Successfully fixed GD services in Pricing Section');
    console.log('Current Service IDs:', updatedServices.map(s => s.id));
  } catch (error) {
