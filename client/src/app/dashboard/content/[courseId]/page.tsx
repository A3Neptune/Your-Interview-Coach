...
export default function CourseDetailPage() {
  ...
  // Cache video URL when lesson selected
  useEffect(() => {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(data.message || "Enrolled!");
        setIsEnrolled(true);
        setShowModal(false);
      } else {
  ...
  const renderContent = () => {
    ...
    // Lock wall — shown in main area when paid + unenrolled
    if (!canWatch) {
      return (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ position: "absolute", inset: 0, backdropFilter: "blur(2px)", background: "rgba(15,23,42,0.45)" }} />
            <div style={{ position: "relative", textAlign: "center", color: "#fff" }}>
              ...
              <p style={{ fontWeight: 700, fontSize: 18, margin: 0 }}>Locked</p>
              <p style={{ fontSize: 13, opacity: 0.75, marginTop: 4 }}>{contents.length} lesson{contents.length !== 1 ? "s" : ""} inside</p>
            </div>
          </div>
          {/* Enroll CTA */}
          <div style={{ padding: "28px 32px", textAlign: "center" }}>
            <h3 style={{ fontSize: 20, fontWeight: 700, color: INK, margin: "0 0 8px", letterSpacing: "-0.02em" }}>
              {isPaidCourse ? `Enroll for ₹${course?.price}` : "Enroll for Free"}
            </h3>
            <p style={{ fontSize: 14, color: MUTED, margin: "0 0 24px", lineHeight: 1.6 }}>
              {isPaidCourse
                ? "Get full access to all lessons, progress tracking, and certificate."
            ...
            <button
              ...
              style={{
                ...
                boxShadow: isEnrolling ? "none" : "0 10px 28px rgba(37,99,235,0.32)",
                transition: "all 0.2s",
              }}
            >
              {isEnrolling ? (
                <><div style={{ width: 16, height: 16, border: "2px solid rgba(255,255,255,0.4)", borderTopColor: "#fff", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} /> Enrolling…</>
              ) : isPaidCourse ? (
                <><Lock style={{ width: 15, height: 15 }} /> Enroll Now — ₹{course?.price}</>
              ) : (
                <><Play style={{ width: 15, height: 15, fill: "#fff" }} /> Enroll for Free</>
              )}
            </button>
          </div>
        </div>
    ...
    switch (selected.contentType) {
      ...
      case "google-sheet": {
        ...
        if (!docUrl) return (
          <div style={{ borderRadius: 16, border: "1px solid #fde68a", background: "#fffbeb", padding: "32px", textAlign: "center" }}>
            <p style={{ color: "#92400e", fontWeight: 600 }}>Invalid lesson link</p>
          </div>
        );
        return (
      ...
      case "video-link": {
        ...
        if (!videoUrl) return (
          <div style={{ borderRadius: 16, border: "1px solid #fde68a", background: "#fffbeb", padding: "32px", textAlign: "center" }}>
            <p style={{ color: "#92400e", fontWeight: 600 }}>Invalid video link</p>
          </div>
        );
        ...
        return (
          <div style={{
            ...
            <div style={{ position: "relative", aspectRatio: "16/9", background: "#0f172a", overflow: "hidden" }}>
              {ytThumb && !isPlaylist ? (
                ...
                <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, color: "rgba(255,255,255,0.35)" }}>
                  <svg viewBox="0 0 90 20" style={{ width: 100, fill: "rgba(255,255,255,0.18)" }}>
                  ...
                  <p style={{ fontSize: 13 }}>{isPlaylist ? "YouTube Playlist" : "YouTube Video"}</p>
                </div>
              )}
            </div>
            ...
            <div style={{ padding: "20px 22px", display: "flex", flexDirection: "column", gap: 12 }}>
              <div>
                <p style={{ fontSize: 16, fontWeight: 700, color: INK, margin: 0, lineHeight: 1.35, letterSpacing: "-0.01em" }}>{selected.title}</p>
                {selected.description && (
                  <p style={{ fontSize: 13, color: MUTED, margin: "6px 0 0", lineHeight: 1.6 }}>{selected.description}</p>
                )}
                {selected.duration > 0 && (
                  <p style={{ fontSize: 12, color: "#94a3b8", fontWeight: 500, margin: "6px 0 0", display: "flex", alignItems: "center", gap: 5 }}>
              ...
              {isEnrolled && (
                <button
                  onClick={() => toggleDone(selected._id)}
                  style={{
                    display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                ...
                >
                  <CheckCircle style={{ width: 14, height: 14, ...(completedItems.has(selected._id) ? { fill: "#059669" } : {}) }} />
                  {completedItems.has(selected._id) ? "Completed — mark as undone" : "Mark as done"}
                </button>
              )}
            </div>
          </div>
      ...
      default:
        return (
          <div style={{ borderRadius: 16, border: "1px solid rgba(29,78,216,0.14)", background: "#fff", padding: "32px", textAlign: "center" }}>
            ...
            {selected.videoUrl && (
              <a href={selected.videoUrl} target="_blank" rel="noopener noreferrer"
                style={{ marginTop: 16, display: "inline-flex", alignItems: "center", gap: 6, color: BRAND, fontWeight: 600, textDecoration: "none", fontSize: 14 }}>
                Open content <ExternalLink style={{ width: 13, height: 13 }} />
              </a>
            )}
          </div>
        );
    ...
    return (
      <div style={{ minHeight: "100vh", background: PAPER, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{
          ...
          border: "3px solid rgba(37,99,235,0.2)", borderTopColor: BRAND,
    ...
    return (
      <div style={{ minHeight: "100vh", background: PAPER, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <Link href="/dashboard/content" style={{ color: BRAND, fontWeight: 600, textDecoration: "none" }}>
            ← Back to courses
          </Link>
        </div>
      </div>
  ...
  return (
    <>
      <style>{`
        ...
        .cd-lesson-scroll::-webkit-scrollbar { width: 4px; }
        .cd-lesson-scroll::-webkit-scrollbar-thumb { background: rgba(29,78,216,0.15); border-radius: 99px; }
      `}</style>
      ...
      <div style={{ minHeight: "100vh", background: PAPER, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0 }}>
          <div style={{ position: "absolute", top: "5%", left: "5%", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle,rgba(29,78,216,0.055) 0%,transparent 70%)", filter: "blur(80px)" }} />
          <div style={{ position: "absolute", bottom: "10%", right: "5%", width: 350, height: 350, borderRadius: "50%", background: "radial-gradient(circle,rgba(8,145,178,0.045) 0%,transparent 70%)", filter: "blur(80px)" }} />
        </div>

        <div style={{ position: "relative", zIndex: 1, maxWidth: 1280, margin: "0 auto", padding: "clamp(16px,4vw,36px) clamp(16px,4vw,32px)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
            <Link
              ...
              style={{
                ...
                fontSize: 13, fontWeight: 600, color: BRAND, textDecoration: "none",
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 20 }} className="lg:grid-cols-[1fr_340px]">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
              ...
              {selected ? renderContent() : (
                <div style={{
                  ...
                  <p style={{ color: MUTED, fontWeight: 600, fontSize: 15 }}>Select a lesson to begin</p>
                </div>
              )}
                ...
                <div style={{
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <h2 style={{ fontSize: "clamp(16px,2vw,20px)", fontWeight: 700, color: INK, margin: 0, letterSpacing: "-0.02em" }}>
                        {selected.title}
                      </h2>
                      {selected.description && (
                        <p style={{ fontSize: 14, color: MUTED, lineHeight: 1.65, margin: "8px 0 0" }}>{selected.description}</p>
                      )}
                      {selected.duration > 0 && (
                        <p style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: BRAND, fontWeight: 600, margin: "10px 0 0" }}>
                          <Clock style={{ width: 13, height: 13 }} /> {selected.duration} minutes
                        </p>
                      )}
                    </div>
                    {isEnrolled && (
                      <div style={{
                        ...
                        color: completedItems.has(selected._id) ? "#059669" : MUTED,
                        background: completedItems.has(selected._id) ? "#f0fdf4" : "rgba(255,255,255,0.9)",
                        border: `1px solid ${completedItems.has(selected._id) ? "#bbf7d0" : "rgba(29,78,216,0.14)"}`,
                        padding: "6px 12px", borderRadius: 99,
                      }}>
                        <CheckCircle style={{ width: 13, height: 13, ...(completedItems.has(selected._id) ? { fill: "#059669" } : {}) }} />
                        {completedItems.has(selected._id) ? "Done" : "Not done"}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <div style={{ flex: 1, minWidth: 120 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                      <span style={{ fontSize: 12, fontWeight: 700, color: INK, letterSpacing: "-0.01em" }}>Your Progress</span>
                      <span style={{ fontSize: 13, fontWeight: 800, color: BRAND }}>{progressPct}%</span>
                    </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
                    <Award style={{ width: 14, height: 14, color: progressPct === 100 ? "#059669" : "#94a3b8" }} />
                    <span style={{ fontSize: 12, color: progressPct === 100 ? "#059669" : MUTED, fontWeight: 600 }}>
                      {completedItems.size}/{contents.length} done
                    </span>
                  </div>
                </div>
              )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{
                ...
                {/* Accent top line */}
                <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: `linear-gradient(90deg, ${BRAND}, ${BRAND_DEEP})` }} />

                <h3 style={{ fontSize: 17, fontWeight: 700, color: INK, margin: "0 0 8px", letterSpacing: "-0.02em", lineHeight: 1.3 }}>
                  {course.title}
                </h3>
                <span style={{
                  display: "inline-block", fontSize: 10.5, fontWeight: 700,
                  color: BRAND, background: "#eff6ff", border: "1px solid #bfdbfe",
                  padding: "3px 10px", borderRadius: 99, textTransform: "capitalize", marginBottom: 12,
                }}>
                  {CAT_LABEL[course.category] ?? course.category.replace(/-/g, " ")}
                </span>

                {course.shortDescription && (
                  <p style={{ fontSize: 13, color: MUTED, lineHeight: 1.65, margin: "0 0 16px" }}>{course.shortDescription}</p>
                )}
                ...
                {isEnrolled ? (
                  <div style={{
                    ...
                    <CheckCircle style={{ width: 15, height: 15, fill: "#059669" }} />
                    Enrolled
                  </div>
                ) : (
                  <button
                    ...
                    style={{
                      ...
                      cursor: isEnrolling ? "not-allowed" : "pointer", fontFamily: "inherit",
                      boxShadow: isEnrolling ? "none" : "0 8px 22px rgba(37,99,235,0.30)",
                      transition: "all 0.2s",
                    }}
                  >
                    {isEnrolling ? (
                      <><div style={{ width: 14, height: 14, border: "2px solid rgba(255,255,255,0.4)", borderTopColor: "#fff", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} /> Enrolling…</>
                    ) : isPaidCourse ? (
                      <><Lock style={{ width: 14, height: 14 }} /> Enroll — ₹{course.price}</>
                    ) : (
                      <><Play style={{ width: 14, height: 14, fill: "#fff" }} /> Enroll for Free</>
                    )}
                  </button>
                )}

                {/* Instructor */}
                <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid rgba(29,78,216,0.08)" }}>
                  <p style={{ fontSize: 10.5, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.09em", margin: "0 0 10px" }}>
                    Instructor
                  </p>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <p style={{ fontSize: 13.5, fontWeight: 700, color: INK, margin: 0 }}>{course.mentorId.name}</p>
                      <p style={{ fontSize: 11.5, color: MUTED, margin: "2px 0 0" }}>{course.mentorId.designation}</p>
                      {course.mentorId.company && (
                        <p style={{ fontSize: 11, color: BRAND, fontWeight: 600, margin: "1px 0 0" }}>{course.mentorId.company}</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <h4 style={{ fontWeight: 700, fontSize: 14, color: INK, margin: 0 }}>
                    Lessons <span style={{ color: "#94a3b8", fontWeight: 500 }}>({contents.length})</span>
                  </h4>
                ...
                {/* List */}
                <div className="cd-lesson-scroll" style={{ maxHeight: 420, overflowY: "auto" }}>
                  {contents.map((c, i) => {
                      ...
                      <button
                        ...
                        style={{
                          ...
                          background: active && !locked
                            ? "rgba(37,99,235,0.07)"
                            : done && !active ? "rgba(16,185,129,0.04)" : "transparent",
                        ...
                        className={!locked ? "hover:bg-blue-50/60" : ""}
                      >
                        {/* Step indicator */}
                        <div style={{ flexShrink: 0, marginTop: 2 }}>
                          {locked ? (
                            <div style={{ width: 20, height: 20, borderRadius: "50%", background: "#f1f5f9", border: "1.5px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            ...
                            <div style={{ width: 20, height: 20, borderRadius: "50%", border: "1.5px solid #cbd5e1", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
                              <span style={{ fontSize: 9.5, fontWeight: 700, color: MUTED }}>{i + 1}</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{
                            fontSize: 13, fontWeight: 600, lineHeight: 1.4, margin: 0,
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3 }}>
                            {c.duration > 0 && (
                              <span style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 11, color: MUTED, fontWeight: 500 }}>
                                <Clock style={{ width: 10, height: 10 }} /> {c.duration}m
                              </span>
                            )}
                            {done && <span style={{ fontSize: 10.5, color: "#10b981", fontWeight: 600 }}>✓ Done</span>}
                          </div>
                        </div>
                      </button>
                    );
                  ...
                  {contents.length === 0 && (
                    <div style={{ padding: "32px 20px", textAlign: "center" }}>
                      <p style={{ color: MUTED, fontSize: 13, fontWeight: 500 }}>No lessons added yet</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ padding: "28px 28px 24px" }}>
                <h3 style={{ fontSize: 20, fontWeight: 700, color: INK, margin: "0 0 20px", letterSpacing: "-0.025em" }}>
                  Enroll in Course
                </h3>
                <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 24 }}>
                  <div style={{ background: "#f8faff", border: "1px solid rgba(29,78,216,0.12)", borderRadius: 12, padding: "14px 16px" }}>
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.07em", margin: "0 0 4px" }}>Course</p>
                    <p style={{ fontWeight: 700, color: INK, margin: 0, fontSize: 14 }}>{course.title}</p>
                  </div>
                  <div style={{ background: "#f8faff", border: "1px solid rgba(29,78,216,0.12)", borderRadius: 12, padding: "14px 16px" }}>
                    <p style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.07em", margin: "0 0 4px" }}>Amount</p>
                    <p style={{ fontSize: 28, fontWeight: 800, color: BRAND, margin: 0, letterSpacing: "-0.03em" }}>₹{course.price}</p>
                  </div>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  ...
                  <button
                    onClick={() => router.push(`/dashboard/checkout/${courseId}`)}
                    style={{
                      flex: 1, padding: "12px 0", borderRadius: 12,
