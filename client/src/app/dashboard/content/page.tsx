...
function CatalogueCard({ course }: { course: Course }) {
  ...
  return (
    ...
    >
      {/* Thumbnail — clean, no overlays or text on top */}
      <div style={{
        position: 'relative', height: 168, flexShrink: 0,
        ...
        {course.thumbnail
          ...
          : <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <BookOpen style={{ width: 48, height: 48, color: 'rgba(255,255,255,0.25)' }} />
            </div>
        }
        {/* Subtle play overlay on hover only */}
        <div style={{
          position: 'absolute', inset: 0,
          background: hovered ? 'rgba(15,23,42,0.18)' : 'transparent',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          {hovered && (
            <div style={{
              ...
              <Play style={{ width: 16, height: 16, fill: BRAND, color: BRAND, marginLeft: 2 }} />
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, padding: '14px 16px 16px', gap: 8 }}>
        ...
        {/* Top row: category chip + type badge */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
          ...
          {isEnrolled
            ...
            : isPaid
              ? <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 99, background: '#1e3a8a', color: '#fff' }}>Paid</span>
              : <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 99, background: '#10b981', color: '#fff' }}>Free</span>
          }
        </div>
        ...
        {/* Price row — only for paid + not enrolled */}
        {isPaid && !isEnrolled && course.price > 0 && (
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}>
            <span style={{ fontSize: 22, fontWeight: 800, color: INK, letterSpacing: '-0.03em' }}>₹{course.price}</span>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {isEnrolled ? (
            ...
            <Link href={`/dashboard/content/${course._id}`} style={{
              ...
              <Eye style={{ width: 11, height: 11 }} /> Preview Free Content
            </Link>
          )}
          <Link
            href={`/dashboard/content/${course._id}`}
