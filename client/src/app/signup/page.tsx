...
export default function SignupPage() {
  ...
  const handleGoogleSuccess = async (tokenResponse: any) => {
    try {
      setIsLoading(true);
      const response = await authAPI.googleLogin(tokenResponse.access_token, 'access_token');
      setAuthToken(response.data.token);
      await fetchUser();
  ...
  return (
    <>
      <style>{`
        ...
        .google-container:hover .google-btn {
          transform: translateY(-1px);
            ...
            {!googleData && (
              <>
                <button
                  ...
                  disabled={isLoading}
                  className="google-btn flex items-center justify-between w-full py-3 px-4 bg-white border border-slate-200 rounded-xl transition-all duration-200 mb-5"
                >
